...
while running:
	...
	for event in pygame.event.get():
		...
		if event.type == QUIT:
			...
			for controller in joysticks:
				jumping[i] = controller.get_button(jumpbutton)
				i+=1
	...
	#new joystick code makes game lag noticeably even when outside of event loop
	for controller in enumerate(joysticks):
		i = controller[0]
		xspeed[i] = controller[1].get_axis(Xax)
		...
		for player in players:
			for m in player:
				if pygame.sprite.collide_rect(p, m):
					if m.rect.left <= p.rect.right and m.rect.right >= p.rect.left:#if on top of 
						if m.rect.bottom <= p.rect.top+10 and m.rect.bottom >= p.rect.top:
							m.y_vel = 0
							m.jumped = False
							m.jump_frames = 0
							m.rect.bottom = p.rect.top+1
						elif m.rect.top >= p.rect.bottom-9 and m.rect.top <= p.rect.bottom:
							m.y_vel = 8
							m.jumped = True
							m.jump_frames = 0
...
#Version History:
	...
	#NOT FINISHED 1.5 - revamping movement to use a loop to ease collision detection

#Bugs:

	#Mario faces does not change direction if movement changes direction without stopping
	#Mario glitches through bottom of platform
	#Mario can't jump while resting on platform
	#Platforms don't exist if bills don't exist
